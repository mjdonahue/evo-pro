...
impl DatabaseManager {
    ...
    pub async fn create_participant(&self, participant: &CreateParticipant) -> Result<Participant> {
        ...
        let row = sqlx::query!(
            r#"INSERT INTO participants (
                    ...
                    display_name, avatar_url, status, metadata
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                RETURNING
                    ...
                    display_name, avatar_url, status as "status: ParticipantStatus", metadata as "metadata: Json<Value>",
    ...
    pub async fn get_participant(&self, id: &Uuid) -> Result<Option<Participant>> {
        ...
        let Some(row) = sqlx::query!(
            r#"SELECT
                id as "id: Uuid", workspace_id as "workspace_id: Uuid", user_id as "user_id: Uuid",
                agent_id as "agent_id: Uuid", contact_id as "contact_id: Uuid",
                display_name, avatar_url, status as "status: ParticipantStatus",
    ...
    pub async fn list_participants(&self, filter: &ParticipantFilter) -> Result<Vec<Participant>> {
        let mut qb: QueryBuilder<Sqlite> = QueryBuilder::new(
            r#"SELECT id, workspace_id, user_id, agent_id, contact_id, display_name, avatar_url, status, metadata, created_at, updated_at FROM participants"#,
        );
        ...
        if let Some(type_) = filter.type_ {
            match type_ {
                ...
                ParticipantType::System => {
                    add_where(&mut qb);
                    qb.push("user_id IS NULL AND agent_id IS NULL AND contact_id IS NULL");
                }
            }
        }
        ...
        if let Some(display_name) = &filter.display_name {
            ...
            qb.push_bind(format!("%{display_name}%"));
        }

        if let Some(avatar_url) = &filter.avatar_url {
            add_where(&mut qb);
            qb.push("avatar_url = ");
            qb.push_bind(format!("%{avatar_url}%"));
        }
        ...
        if filter.active_only.is_some() {
            add_where(&mut qb);
            qb.push("status = ");
            qb.push_bind(ParticipantStatus::Active);
        }

        qb.push(" ORDER BY display_name ASC, created_at DESC");
        ...
        Ok(qb
            ...
            .map(|r| {
                Ok(Participant {
                    ...
                    type_: ParticipantType::from_id_triplet(
                        r.try_get("user_id")?,
                        r.try_get("agent_id")?,
                        r.try_get("contact_id")?,
                    ),
