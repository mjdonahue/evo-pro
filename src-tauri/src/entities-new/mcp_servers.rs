...
impl DatabaseManager {
    ...
    pub async fn create_mcp_server(&self, server: &McpServer) -> Result<McpServer> {
        debug!("Creating MCP server with ID: {}", server.id);

        let metadata = server.metadata.as_deref();
        ...
        Ok(sqlx::query_as!(
            ...
            r#"INSERT INTO mcp_servers (
                    ...
                    id AS "id: _", name, description, host, port, api_key, auth_token,
                    is_active, is_default, metadata AS "metadata: _", created_at AS "created_at: _", updated_at AS "updated_at: _""#,
        id,
        server.name,
        server.description,
        server.host,
